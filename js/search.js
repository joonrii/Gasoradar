(() => {
  ...
  function render(items, select){
    state.results=items; state.active=-1;
    if(!items.length){ $('suggestions').innerHTML='<div class="suggestion"><div class="suggestion-main"><div class="suggestion-name">No encontramos esa localidad</div><div class="suggestion-sub">Prueba con otra ciudad o municipio.</div></div></div>'; show(); return; }
    $('suggestions').innerHTML=items.map((x,i)=>'<button type="button" class="suggestion" data-i="'+i+'"><div class="suggestion-main"><div class="suggestion-name">'+esc(x.nombre)+'</div><div class="suggestion-sub">'+esc(x.provincia)+'</div></div><div class="suggestion-count">'+x.n+' est.</div></button>').join('');
    [...$('suggestions').querySelectorAll('[data-i]')].forEach(b=>b.addEventListener('click',()=>select(Number(b.dataset.i)))); show();
  }
