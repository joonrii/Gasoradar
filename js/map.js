(() => {
  let map=null,cluster=null,moveHandler=null;
  ...
  function init(){if(map)return map;map=L.map('map',{zoomControl:false,minZoom:5,maxZoom:18,preferCanvas:true});L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'© OpenStreetMap'}).addTo(map);cluster=L.markerClusterGroup({maxClusterRadius:48,disableClusteringAtZoom:14,showCoverageOnHover:false,spiderfyOnMaxZoom:true,removeOutsideVisibleBounds:true,animate:true,chunkedLoading:true,chunkInterval:120,chunkDelay:20,iconCreateFunction:clusterIcon});map.addLayer(cluster);if(moveHandler)map.on('moveend',moveHandler);return map}
